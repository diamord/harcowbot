...
module.exports = {
	...
	async execute(user) {
        ...
        const content = `${data.description.replace(`{username}`, user.user.username).replace(`{guilduser}`, user.guild.memberCount).replace(`{line}`, '\n')}`

        const embed = new EmbedBuilder()
            .setColor('Random')
            .setAuthor({ name: `${user.user.username} 已離開!!`, iconURL: user.user.avatarURL() })
